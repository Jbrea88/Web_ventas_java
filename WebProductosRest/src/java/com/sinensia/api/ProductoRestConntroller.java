...
public class ProductoRestConntroller extends HttpServlet {
    ...
    protected void doDelete(HttpServletRequest request, HttpServletResponse response)
        ...
        BufferedReader bufRead = request.getReader();
        String indiceLeido = bufRead.readLine();
        int indiceEntero = Integer.parseInt(indiceLeido);
        bufRead.close();
        ...
        if (listaLeida.size() >= indiceEntero) {
            sps.eliminar(indiceEntero);
            mensaje = "eliminado";
        } else {
            mensaje = "no se puede eliminar porque ya existe el producto";
        }
        Gson gson = new Gson();
        String jsonRespuesta = gson.toJson(mensaje);
        escritorRespuesta.println(jsonRespuesta);
